...
Выборка из 30 таких снимков проверена вручную (`results/non_frontal_check.csv`,
столбец `ok`): поворот головы подтверждён в 28 случаях, два снимка (`unsure`) с
углами вблизи порога 15° оценить на глаз не удалось. Знак угла на всех снимках
совпадает с конвенцией: yaw > 0 означает поворот лица к левому краю кадра.
...
требование. Оценка пишет `results/raw_predictions.csv` и `results/metrics.json`.
Итог: `results/table2.csv` (метрики по требованиям), `results/table2_integral.csv`
(FAR, FRR, время, свойства заключений) и четыре рисунка в `results/figures/`
(JPEG, 300 dpi). Консистентность и полнота заключений считаются по обязательным
требованиям: утверждения о нарушениях берутся из итоговых строк заключения.

Текущие результаты получены шаблонным генератором заключений и содержат только
предложенный метод: сравнение с VLM и измерение LLM-пути вербализации требуют
ключа API и выполняются отдельно (см. ниже). Метрики, полученные заглушкой,
пишутся в `results/mock/`, помечаются `is_mock` и игнорируются `plots.py`, так что
в рисунки и таблицы они попасть не могут.

## Прогон с ключом

Когда появится ключ Anthropic API, два эксперимента выполняются теми же
командами без изменений кода (PowerShell, из каталога проекта):

```
$env:ANTHROPIC_API_KEY = "sk-ant-..."
uv run python -m experiments.run_vlm_baseline --limit 100
uv run python -m experiments.run_llm_conclusions --limit 100
uv run python -m experiments.plots
```

Первая команда прогоняет zero-shot VLM-бейзлайн (`claude-sonnet-4-6`, изображение
плюс список требований, структурированный ответ) на стратифицированной выборке
из 100 снимков тест-сплита и пишет `results/vlm_metrics.json` и
`results/vlm_predictions.csv`. Вторая на той же выборке генерирует заключения
языковой моделью по JSON-отчётам и пишет `results/llm_conclusions_metrics.json`:
долю заключений, прошедших сверку с первой попытки, долю регенераций и долю
откатов на шаблон, а также тексты в `results/llm_conclusions/`. Третья
перестраивает рисунки 3–4 и таблицы уже с колонками `vlm_*` и `llm_*`. Выборка
детерминирована (seed 42), ответы кэшируются в `results/llm_cache/`, поэтому
повторные запуски бесплатны. Ориентировочно 200 запросов, порядка одного-двух
долларов. Флаг `--mock` у обоих скриптов нужен только для отладки пайплайна.
